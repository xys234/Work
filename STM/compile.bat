...
REM pyinstaller --distpath %DISTPATH% --workpath %WORKPATH%  network_prep.spec
REM COPY /Y /D %DISTPATH%\network_prep\*.*			%STMA_SOFTWARE_PATH%\.

pyinstaller --distpath %DISTPATH% --workpath %WORKPATH%  stma.spec
COPY /Y /D %DISTPATH%\stma\stma.*			%STMA_SOFTWARE_PATH%\.	
COPY /Y /D %DISTPATH%\stma\stma.*			L:\DCS\Projects\_Legacy\60563434_SWIFT\400_Technical\SWIFT_Workspace\Software\STM_A\.	
