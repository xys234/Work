...
ECHO Y | pyinstaller --distpath %DISTPATH% --workpath %WORKPATH%  trip_prep.spec
COPY /Y /D %DISTPATH%\trip_prep\trip_prep.*			%STMA_SOFTWARE_PATH%\.	
COPY /Y /D %DISTPATH%\trip_prep\trip_prep.*			L:\DCS\Projects\_Legacy\60563434_SWIFT\400_Technical\SWIFT_Workspace\Software\STM_A\.
...
COPY /Y /D %DISTPATH%\convert_trips\convert_trips.*						%STMA_SOFTWARE_PATH%\.	
COPY /Y /D %DISTPATH%\convert_trips\convert_trips.*						L:\DCS\Projects\_Legacy\60563434_SWIFT\400_Technical\SWIFT_Workspace\Software\STM_A\.

REM ECHO Y | pyinstaller --distpath %DISTPATH% --workpath %WORKPATH%  network_prep.spec
REM COPY /Y /D %DISTPATH%\network_prep\network_prep.*			%STMA_SOFTWARE_PATH%\.
